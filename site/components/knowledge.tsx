  ...
  return (
    <Section id="knowledge" tone="green" kicker={k.kicker} title={k.title} lede={k.lede}>
      <div className="grid gap-10 lg:grid-cols-2 lg:gap-16">
        <Reveal>
          <div className="rounded-xl border border-line bg-card p-4 font-ui shadow-xl shadow-ink/5">
        ...
        <div className="space-y-8">
          {k.notes.map((n, i) => (
            <Reveal key={n.title} delay={i * 80}>
              ...
              <p className="mt-1 text-ink-2">{n.body}</p>
            </Reveal>
          ))}
          <Reveal>
            <Image src={k.screenshot.src} alt={k.screenshot.alt} width={k.screenshot.width} height={k.screenshot.height} className="rounded-xl border border-line" />
          </Reveal>
        </div>
      </div>
    </Section>
